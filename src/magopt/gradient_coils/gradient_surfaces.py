...
class surface:
    ...
    def visualize_surface(self,
        ...
        # Query surface
        u = torch.linspace(0, 1, num_u, device=self.device)
        v = torch.linspace(0, 1, num_v, device=self.device)
        u, v = torch.meshgrid(u, v, indexing='ij')
        xyz = self.to_xyz(u, v).cpu() * 1e2
...
class elliptical_frustum(surface):
    ...
    def sample_edges(self,
                     num_pts: int = 100):
        u_lin = torch.linspace(0, 1, num_pts//2, device=self.device)
...
class planar_curved_surface(surface):
    ...
    def __init__(self,
        ...
        height_curve : float
            shape (3,) representing the height of the surface in orthogonal (to u and v)direction
        poly_degree : int
            shape (3,) representing the degree of the polynomial curve in the orthogonal direction.
        **kwargs : dict, optional
            Additional keyword arguments for general surface constructor
        """
        # Call general surface constructor
...
class planar_arb_srface(surface):
    """
    Planar curved surface.
    ...
    def __init__(self,
        ...
        ----
        func : callable
            function that gives the orthogonal height as a function of u.
        dfdu : callable
            derivative of func with respect to u.
        u_axis : torch.Tensor
            shape (3,) representing the first coordinate axis.
        v_axis : torch.Tensor
            shape (3,) representing the second coordinate axis.
    ...
    def dxyz_duv(self, 
        ...
        # Add curvature term
        ru += self.o_axis * self.dfdu(u)[..., None]
...
class parametric_cyl_surface(surface):
    """
    Planar parametric surface.
    ...
    def __init__(self,
                 ...
                 db1_du: callable,
                 db2_du: callable,
                 b1_axis: torch.Tensor = torch.tensor([1, 0, 0]),
                 b2_axis: torch.Tensor = torch.tensor([0, 1, 0]),
                 v_axis: torch.Tensor = torch.tensor([0, 0, 1]),
                 center: torch.Tensor = torch.tensor([0, 0, 0]),
                 width_v: float = 1.0,
                 **kwargs):
        """
        ...
        ----
        b1_of_u: callable
            function that gives the first basis vector as a function of u.
        b2_of_u: callable
            function that gives the second basis vector as a function of u.
        db1_du: callable
            derivative of b1_of_u with respect to u.
        db2_du: callable
            derivative of b2_of_u with respect to u.
        b1_axis: torch.Tensor
            shape (3,) representing the first basis vector.
        b2_axis: torch.Tensor
            shape (3,) representing the second basis vector.
        **kwargs : dict, optional
            Additional keyword arguments for general surface constructor
        """
        # Call general surface constructor
        ...
        # Store axes
        self.b1_axis = b1_axis.to(self.device)
        self.b2_axis = b2_axis.to(self.device)
        self.v_axis = v_axis.to(self.device)
        self.center = center.to(self.device)
        self.w_v = width_v
        
        # Store funcs
        self.b1_of_u = b1_of_u
        self.b2_of_u = b2_of_u
        self.db1_du = db1_du
        self.db2_du = db2_du
    ...
    def to_xyz(self, 
        ...
        # v axis
        v_cent = (v - 0.5)[..., None]
        crds = self.w_v * self.v_axis * v_cent
        
        # u axis
        us = u[..., None]
        crds += self.b1_of_u(us) * self.b1_axis
        crds += self.b2_of_u(us) * self.b2_axis
    ...
    def dxyz_duv(self, 
                 ...
                 v: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        us = u[..., None]
        ru = self.db1_du(us) * self.b1_axis + \
             self.db2_du(us) * self.b2_axis
        rv = self.w_v * self.v_axis * torch.ones_like(v)[..., None]
